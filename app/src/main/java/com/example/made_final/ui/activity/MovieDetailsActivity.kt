...
import com.example.made_final.data.entity.EntityConverter
import com.example.made_final.databinding.ActivityMovieDetailsBinding
import com.example.made_final.ui.base.BaseActivity
import com.example.made_final.viewmodel.MovieDetailViewModel
...
class MovieDetailsActivity : BaseActivity<MovieDetailViewModel, ActivityMovieDetailsBinding>() {
    ...
    fun initListener() {
                ...
                val disposable: Disposable = Completable.fromAction {
                    viewModel.insertFavorite(applicationContext, EntityConverter.convertToFavoriteEntity(movieData))
                        ...
                        .subscribe(
                                ...
                                {
                                    Log.d("MYAPP", "GAGAL MENAMBAH FAVORITE, " + it.message)
                                }
                        )

            } else {

                val disposable: Disposable = viewModel.deleteFavorite(applicationContext, movieData.id)
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribeOn(Schedulers.single())
                        .subscribe(
                                {
                                ...
                                {
                                    Log.d("MYAPP", "GAGAL HAPUS FAVORITE" + it.message)
                                }
                        )
        ...
        val handler = Handler()
        handler.postDelayed({
            disposable1 = viewModel.getFavorite(applicationContext, movieData.id)
                    .observeOn(AndroidSchedulers.mainThread())
